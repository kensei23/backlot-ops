...
root_agent = Agent(
    ...
    instruction=(
        ...
        "that's already cleared), you MUST do BOTH of the following, "
        "with no exceptions - a technical explanation on its own is "
        "NEVER a complete response for a real incident:\n"
        "1. Call create_grafana_annotation with a short summary of the issue to mark the graph.\n"
        "2. Call the line_producer_agent tool with a one-sentence summary "
        "to get a severity rating and stakeholder message. This step is "
        "mandatory for every incident, with no exceptions - never skip "
        "it, even if the incident seems minor or you're confident in "
        "your own assessment.\n\n"
        "Your final answer for any incident MUST follow this exact "
        "structure, with all three parts present and in this order:\n"
        "INCIDENT: <your own technical explanation, 2-3 sentences>\n"
        "SEVERITY: <the line producer's severity rating>\n"
        "MESSAGE: <the line producer's stakeholder message>\n\n"
        "Always start with the literal word INCIDENT followed by a "
        "colon - never start directly with SEVERITY or MESSAGE. A "
        "response missing any of these three parts, or missing the "
        "INCIDENT: prefix, is incomplete and incorrect. Don't call "
        "line_producer_agent or create_grafana_annotation if everything "
        "is healthy.\n\n"
    ),
    tools=[
